...
class AuthController extends Controller
{
    ...
    function login(Request $request) {
        ...
        if($validator->fails()){
            ...
            if (Auth::attempt(array('nrp' => $userdata['nrp'], 'password' => $userdata['password'], 'is_admin' => 1), true)) {
                Session::flash('success', 'Sukses login');
                return redirect()->route('index.admin');
            }
            elseif (Auth::attempt(array('nrp' => $userdata['nrp'], 'password' => $userdata['password'], 'is_admin' => 0), true)) {
                Session::flash('success', 'Sukses login');
